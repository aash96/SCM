...
VERSION="1"     #increment this while upgrading
...
chaincodeInvoke(){
    ...
    peer chaincode invoke -o localhost:7050 \
        ...
        -c '{"function": "CreateCert","Args":["Cert-ABCDEEE", "Kang", "Ravi", "India"]}'
        # actual cert number should be CERTinteger
}
...
chaincodeQuery(){
    ...
    # Query all Certs
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["queryAllCerts"]}'

    # Query by Cert id
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["queryCert", "Cert-ABCDEEE"]}'
}
presetup
packageChaincode
installChaincode
